...
public class PrefabMapItemPainter : MonoBehaviour
{
    ...
    public void Drag(Vector3 mouse, PrefabMapItemEditor.TileOperation op)
    {
        Resize();
        if (tileobs == null) { Restore(); }
        if (this.ValidCoords((int)cursor.x, (int)cursor.y))
        {
            if (op == PrefabMapItemEditor.TileOperation.Sampling)
            {
            ...
            else
            {
                ...
                if (op == PrefabMapItemEditor.TileOperation.Drawing)
                {
    ...
    public void Compile()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                PrefabItemMapTile tempTile = tileobs[j, i].GetComponent<PrefabItemMapTile>();
                items.Add(new Vector3Int(j, i, tempTile.itemIndex));
                if (tempTile.itemIndex != 2
                    && tilePremade.mapTiles[j + i * width].z > 1)
                {
                    Debug.LogError("Item Spawns in wall at: " + j + " " + i);
                }
            }
    ...
    public void LoadPrefabMapTile()
    {
        ...
        Vector3Int tileInfo;
        for (int i = 0; i < tilePremade.items.Count; i++)
        {
            tileInfo = tilePremade.items[i];
            o = CreatePrefab(resourceManager.itemsPrefabs[tileInfo.z], new Vector3(), new Quaternion(0, 0, 0, 1f));
            o.transform.parent = tiles.transform;
    ...
    public void BlankCanvas()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                GameObject o;
                o = CreatePrefab(resourceManager.itemsPrefabs[2], new Vector3(), new Quaternion(0, 0, 0, 1f));
                o.transform.parent = tiles.transform;
...
#if UNITY_EDITOR
[CustomEditor(typeof(PrefabMapItemPainter))]
public class PrefabMapItemEditor : Editor
{
    ...
    {
        PrefabMapItemPainter me = (PrefabMapItemPainter)target;
    ...
    private bool AmHovering(Event e)
    {
        ...
        if (Physics.Raycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), out hit, Mathf.Infinity) &&
                hit.collider.GetComponentInParent<PrefabMapItemPainter>() == me)
        {
