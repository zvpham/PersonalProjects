...
public class WorldMapTilePainter : MonoBehaviour
{
    ...
    public void Drag(Vector3 mouse, WorldMapTileLayerEditor.TileOperation op)
    {
        Resize();
        if (tileobs == null) { Restore(); }
        if (this.ValidCoords((int)cursor.x, (int)cursor.y))
        {
            if (op == WorldMapTileLayerEditor.TileOperation.Sampling)
            {
            ...
            else
            {
                ...
                if (op == WorldMapTileLayerEditor.TileOperation.Drawing)
                {
...
#if UNITY_EDITOR
[CustomEditor(typeof(WorldMapTilePainter))]
public class WorldMapTileLayerEditor : Editor
{
    ...
    {
        WorldMapTilePainter me = (WorldMapTilePainter)target;
    ...
    private bool AmHovering(Event e)
    {
        ...
        if (Physics.Raycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), out hit, Mathf.Infinity) &&
                hit.collider.GetComponentInParent<WorldMapTilePainter>() == me)
        {
