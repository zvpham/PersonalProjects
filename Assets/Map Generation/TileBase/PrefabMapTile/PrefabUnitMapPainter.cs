...
public class PrefabUnitMapPainter : MonoBehaviour
{
    ...
    public void Drag(Vector3 mouse, PrefabUnitMapEditor.TileOperation op)
    {
        Resize();
        if (tileobs == null) { Restore(); }
        if (this.ValidCoords((int)cursor.x, (int)cursor.y))
        {
            if (op == PrefabUnitMapEditor.TileOperation.Sampling)
            {
            ...
            else
            {
                ...
                if (op == PrefabUnitMapEditor.TileOperation.Drawing)
                {
...
#if UNITY_EDITOR
[CustomEditor(typeof(PrefabUnitMapPainter))]
public class PrefabUnitMapEditor : Editor
{
    ...
    {
        PrefabUnitMapPainter me = (PrefabUnitMapPainter)target;
    ...
    private bool AmHovering(Event e)
    {
        ...
        if (Physics.Raycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), out hit, Mathf.Infinity) &&
                hit.collider.GetComponentInParent<PrefabUnitMapPainter>() == me)
        {
