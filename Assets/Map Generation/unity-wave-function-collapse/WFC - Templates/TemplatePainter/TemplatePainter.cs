...
public class TemplatePainter : MonoBehaviour
{
    public List<GameObject> templatePainterTilePrefabs;
    ...
    public void Drag(Vector3 mouse, WFCTemplatePainterEditor.TileOperation op)
    {
        Resize();
        if (tileobs == null) { Restore(); }
        if (this.ValidCoords((int)cursor.x, (int)cursor.y))
        {
            if (op == WFCTemplatePainterEditor.TileOperation.Sampling)
            {
            ...
            else
            {
                ...
                if (op == WFCTemplatePainterEditor.TileOperation.Drawing)
                {
    ...
    public void Compile()
    {
        ...
        List<Vector3Int> templateValues = new List<Vector3Int>();
        string debugWord = "";
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                WFCTemplateTile tempTile = tileobs[j, i].GetComponent<WFCTemplateTile>();
                templateValues.Add(new Vector3Int(j, i, tempTile.tileValue));
                debugWord += tempTile.tileValue + " ";
            }
        ...
        template.templatePositions = templateValues;
    ...
    public void LoadPrefabMapTile()
    {
        Clear();
        GameObject o;
        // TIleINfO - <x, y, WFCTemplateTileIndex>
        Vector3Int tileInfo;
        for (int i = 0; i < template.templatePositions.Count; i++)
        {
            tileInfo = template.templatePositions[i];
            o = CreatePrefab(templatePainterTilePrefabs[tileInfo.z], new Vector3(), new Quaternion(0, 0, 0, 1f));
            o.transform.parent = tiles.transform;
        ...
        public void FillCanvas()
    {
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                GameObject o;
                o = CreatePrefab(templatePainterTilePrefabs[1], new Vector3(), new Quaternion(0, 0, 0, 1f));
                o.transform.parent = tiles.transform;
...
#if UNITY_EDITOR
[CustomEditor(typeof(TemplatePainter))]
public class WFCTemplatePainterEditor : Editor
{
    ...
    {
        TemplatePainter me = (TemplatePainter)target;
    ...
    private bool AmHovering(Event e)
    {
        ...
        if (Physics.Raycast(HandleUtility.GUIPointToWorldRay(Event.current.mousePosition), out hit, Mathf.Infinity) &&
                hit.collider.GetComponentInParent<TemplatePainter>() == me)
        {
