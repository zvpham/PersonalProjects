...
[CreateAssetMenu(menuName = "Action/FlamingWeapons")]
public class FlamingWeapon : Action
{
    ...
    public override void SelectAction(Unit self)
    {
        ...
        AttackData newAttackData = new AttackData(self);
        newAttackData.AddStatus(status);

        RangedTargetingData rangedTargetingData = new RangedTargetingData(self, this, true, false, false,  self.currentMajorActionsPoints, amountOfActionPointsUsed, effectiveRange, maxRange, 3,
            newAttackData, null);
    ...
    public override void ConfirmAction(ActionData actionData)
    {
        ...
        if (actionData.affectedUnits.Count == 0)
        {
            Debug.LogError("Action Called and Confirmed without any targets: " + name);
            movingUnit.UseActionPoints(0);
        }

        bool failedToCompleteActionForAtLeastOneUnit = false;
        for(int i = 0; i < actionData.affectedUnits.Count; i++)
        {
            Unit targetUnit = actionData.affectedUnits[i];
            ...
            {
                Debug.Log("Hit");
                ActionStatusData actionStatusData = new ActionStatusData(targetUnit, 8);
                status.AddStatus(actionStatusData);
            }
            else
            {
                failedToCompleteActionForAtLeastOneUnit = true;
            }
        } 
        
        if(failedToCompleteActionForAtLeastOneUnit)
        {
            Debug.LogWarning("Tried to use Ranged Attacked, but failed for at least one unit");
        }
